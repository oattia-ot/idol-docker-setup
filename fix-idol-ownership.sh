...
# Works on Ubuntu and macOS (stat format differs; OS is auto-detected).
#
...
fix_idol_base_ownership() {
    ...
    if command -v idol_stat_owner >/dev/null 2>&1; then
        before="$(idol_stat_owner "$base" 2>/dev/null || echo unknown)"
    else
        before="$(stat -c '%U:%G' "$base" 2>/dev/null || stat -f '%Su:%Sg' "$base" 2>/dev/null || echo unknown)"
    fi
    ...
    if command -v idol_stat_owner >/dev/null 2>&1; then
        after="$(idol_stat_owner "$base" 2>/dev/null || echo unknown)"
    else
        after="$(stat -c '%U:%G' "$base" 2>/dev/null || stat -f '%Su:%Sg' "$base" 2>/dev/null || echo unknown)"
    fi
