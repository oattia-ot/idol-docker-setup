...
# UTILITIES
# =============================================================================
if [ -f "$BASE_IDOL_SETUP_DIR/module/os-compat.sh" ]; then
    # shellcheck source=/dev/null
    source "$BASE_IDOL_SETUP_DIR/module/os-compat.sh"
    idol_os_banner
fi
...
diagnose_docker_installation() {
    ...
    if [ "${IDOL_OS_FAMILY:-}" = "macos" ]; then
        if [ -d /Applications/Docker.app ]; then
            echo -e " ${GREEN}✓ Docker Desktop app found (/Applications/Docker.app)${NC}"
        elif command -v brew &> /dev/null && brew list --cask docker &>/dev/null; then
            echo -e " ${GREEN}✓ Docker Desktop installed via Homebrew cask${NC}"
        else
            echo -e " ${YELLOW}⚠ Docker CLI present; Docker Desktop app not detected${NC}"
        fi
    elif command -v snap &> /dev/null && snap list docker &> /dev/null 2>&1; then
    ...
    echo " ----------------------"
    if command -v idol_service_status >/dev/null 2>&1; then
        idol_service_status docker.service 10
    elif command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files docker.service &> /dev/null; then
        sudo systemctl status docker.service --no-pager --lines=10 || true
    else
        echo -e " ${YELLOW}⚠ systemd not available on this host (${IDOL_HOST_OS:-unknown})${NC}"
        docker info 2>/dev/null | head -n 8 || echo -e " ${RED}✗ Docker daemon not reachable${NC}"
    fi
    echo ""
    echo -e "${BOLD}${YELLOW}4. Containerd Status:${NC}"
    echo " ------------------"
    if [ "${IDOL_OS_FAMILY:-}" = "macos" ]; then
        echo -e " ${YELLOW}ℹ containerd is managed inside Docker Desktop on macOS${NC}"
    elif command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files containerd.service &> /dev/null; then
    ...
    if command -v idol_mem_info >/dev/null 2>&1; then
        idol_mem_info
    elif command -v free >/dev/null 2>&1; then
        free -h | grep "^Mem:"
    else
        sysctl -n hw.memsize 2>/dev/null || echo "n/a"
    fi
    echo ""
}
...
install_docker() {
    ...
    log "${CALLING_SCRIPT} ${YELLOW}Installing Docker for ${IDOL_HOST_OS:-$(uname -s)}...${NC}"
    if command -v idol_install_docker >/dev/null 2>&1; then
        if idol_install_docker; then
            log "${CALLING_SCRIPT} ✅${GREEN}Docker installed successfully.${NC}"
            return 0
        fi
        log "${CALLING_SCRIPT} ❌${RED}Docker installation failed on ${IDOL_HOST_OS}${NC}"
        diagnose_docker_installation
        return 1
    fi
    # Fallback Ubuntu path if os-compat is missing
    sudo apt update -y
...
install_docker_compose() {
    ...
    if command -v idol_install_docker_compose >/dev/null 2>&1; then
...
validate_idol_components() {
    ...
    if ! command_exists java; then
        ...
        if prompt_yn "Do you want to install openjdk-21-jdk now?" "Y"; then
            if command -v idol_install_java21 >/dev/null 2>&1; then
                idol_install_java21 || error_exit "JAVA installation failed."
            else
