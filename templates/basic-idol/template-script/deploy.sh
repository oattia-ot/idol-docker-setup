...
if [ $COMPOSE_EXIT_CODE -eq 0 ]; then
    ...
    if [ "$IS_UP_COMMAND" = true ]; then
        ...
        if [ -z "$USER_ROLE_SCRIPT" ]; then
            ...
            echo -e "${RED}${BOLD}✗${NC} Community user/role setup failed."
            echo -e "${YELLOW}  ⚠${NC} Rerun standalone:"
            echo -e "      ${CYAN}python3 ${USER_ROLE_SCRIPT} --host ${COMMUNITY_HOST} --port ${COMMUNITY_PORT} --insecure -y${NC}"
        fi
        echo -e "${CYAN}${BOLD}───────────────────────────────────────────────────────${NC}"
    fi
else
