...
# Resolve this script's directory and run from there so every relative path
# (compose files, pre-setup.sh, ssl certs, helper scripts) is stable even if
# the caller invoked deploy.sh from another working directory.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
cd "${SCRIPT_DIR}" || {
    echo -e "${RED}Failed to cd into script directory: ${SCRIPT_DIR}${NC}" >&2
    exit 1
}
...
# Safely source the IDOL SSL passwords file (script-relative, not CWD-relative)
PASSWORD_FILE="${SCRIPT_DIR}/../../env/.idol-ssl-passwords.env"
...
# ─────────────────────────────────────────────────────────────────────────
PRE_SETUP_FILE="${SCRIPT_DIR}/pre-setup.sh"
...
if [ $COMPOSE_EXIT_CODE -eq 0 ]; then
    ...
    # ── Copy NiFi connectors & basic-idol packages into extensions ────────────
    if [ "$IS_UP_COMMAND" = true ]; then
        ...
        echo ""
        echo -e "${CYAN}${BOLD}───────────────────────────────────────────────────────${NC}"
        echo -e "${YELLOW}${BOLD}Configuring IDOL Community users and roles...${NC}"

        WAIT_DATAADMIN_SCRIPT="${SCRIPT_DIR}/wait-dataadmin-for-log.sh"
        if [ -f "${WAIT_DATAADMIN_SCRIPT}" ]; then
            echo -e "${BLUE}  →${NC} Waiting for IDOL Data Admin UI to become ready..."
            chmod +x "${WAIT_DATAADMIN_SCRIPT}" 2>/dev/null || true
            if "${WAIT_DATAADMIN_SCRIPT}"; then
                echo -e "${GREEN}${BOLD}✓${NC} Data Admin UI is ready."
            else
                echo -e "${YELLOW}  ⚠${NC} Wait script failed — continuing with user/role setup anyway."
            fi
        else
            echo -e "${YELLOW}  ⚠${NC} wait-dataadmin-for-log.sh not found — skipping wait."
        fi

        # Prefer a host that is reachable from THIS shell (the Docker host),
        # not a compose-network DNS name like idol-docker-host.
        # EXTRA_IP_SANS_ENV is used only when it differs from IDOL_NET_HOST_IP
        # (e.g. a public IP on the certificate SAN list).
        if [ -n "${EXTRA_IP_SANS_ENV:-}" ] && [ -n "${IDOL_NET_HOST_IP:-}" ] && [ "${EXTRA_IP_SANS_ENV}" != "${IDOL_NET_HOST_IP}" ]; then
            export COMMUNITY_HOST="${EXTRA_IP_SANS_ENV}"
            echo -e "${BLUE}  →${NC} COMMUNITY_HOST set from EXTRA_IP_SANS_ENV → ${GREEN}${COMMUNITY_HOST}${NC}"
        elif [ -n "${IDOL_NET_HOST_IP:-}" ]; then
            export COMMUNITY_HOST="${IDOL_NET_HOST_IP}"
            echo -e "${BLUE}  →${NC} COMMUNITY_HOST set from IDOL_NET_HOST_IP → ${GREEN}${COMMUNITY_HOST}${NC}"
        else
            export COMMUNITY_HOST="127.0.0.1"
            echo -e "${BLUE}  →${NC} COMMUNITY_HOST defaulted to ${GREEN}${COMMUNITY_HOST}${NC} (host-local, not idol-docker-host)"
        fi
        ...
        # Prefer an absolute cert path so Python is not sensitive to CWD.
        DEFAULT_COMMUNITY_CERT="${SCRIPT_DIR}/ssl/intermediate/certs/ca-chain.cert.pem"
        if [ -n "${COMMUNITY_CERT:-}" ] && [ -f "${COMMUNITY_CERT}" ]; then
            :
        elif [ -f "${DEFAULT_COMMUNITY_CERT}" ]; then
            export COMMUNITY_CERT="${DEFAULT_COMMUNITY_CERT}"
        else
            export COMMUNITY_CERT="${COMMUNITY_CERT:-${DEFAULT_COMMUNITY_CERT}}"
        fi
        ...
        USER_ROLE_SCRIPT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/../create-users-roles.py"

        # Derive subtype-aware port-env-name / cert-subdir instead of hardcoding basic-idol.
        PORT_ENV_NAME_ARG=""
        CERT_SUBDIR_ARG="data-admin"
        if [[ -n "${IDOL_DEPLOYMENT_SUBTYPE:-}" ]]; then
            PORT_ENV_NAME_ARG="PORT_$(echo "$IDOL_DEPLOYMENT_SUBTYPE" | tr 'a-z-' 'A-Z_')_COMMUNITY"
            CERT_SUBDIR_ARG="${IDOL_DEPLOYMENT_SUBTYPE}"
        fi

        echo -e "${YELLOW}  ⚠ Execution script: python3 ${USER_ROLE_SCRIPT}${NC}"

        if [ ! -f "$USER_ROLE_SCRIPT" ]; then
            echo -e "${RED}${BOLD}✗${NC} create-users-roles.py not found at: ${CYAN}${USER_ROLE_SCRIPT}${NC}"
            echo -e "${YELLOW}  ⚠${NC} Place create-users-roles.py in: ${CYAN}$(cd "${SCRIPT_DIR}/.." && pwd)${NC}"
        elif ! command -v python3 >/dev/null 2>&1; then
            echo -e "${RED}${BOLD}✗${NC} python3 is not installed or not in PATH."
        else
            echo -e "${BLUE}  →${NC} CWD=$(pwd)"
            echo -e "${BLUE}  →${NC} python3=$(command -v python3)"
            echo -e "${BLUE}  →${NC} HOST=${GREEN}${COMMUNITY_HOST}${NC} PORT=${GREEN}${COMMUNITY_PORT}${NC}"
            echo -e "${BLUE}  →${NC} IDOL_DEPLOYMENT_SUBTYPE=${GREEN}${IDOL_DEPLOYMENT_SUBTYPE:-}${NC}"
            echo -e "${BLUE}  →${NC} PORT_BASIC_IDOL_COMMUNITY=${GREEN}${PORT_BASIC_IDOL_COMMUNITY:-}${NC}  PORT_DATA_ADMIN_COMMUNITY=${GREEN}${PORT_DATA_ADMIN_COMMUNITY:-}${NC}"
            if [ -f "${COMMUNITY_CERT}" ]; then
                echo -e "${BLUE}  →${NC} cert file ${GREEN}exists${NC}"
            else
                echo -e "${YELLOW}  ⚠${NC} cert file ${YELLOW}not found${NC} at ${COMMUNITY_CERT}"
            fi

            # Community can still be coming up after Data Admin is "ready".
            echo -e "${BLUE}  →${NC} Waiting for Community on ${GREEN}${COMMUNITY_HOST}:${COMMUNITY_PORT}${NC}..."
            COMMUNITY_READY=false
            for i in $(seq 1 30); do
                if command -v nc >/dev/null 2>&1 && nc -z "${COMMUNITY_HOST}" "${COMMUNITY_PORT}" >/dev/null 2>&1; then
                    COMMUNITY_READY=true
                    break
                fi
                # bash /dev/tcp fallback when nc is missing
                if (echo > "/dev/tcp/${COMMUNITY_HOST}/${COMMUNITY_PORT}") >/dev/null 2>&1; then
                    COMMUNITY_READY=true
                    break
                fi
                echo -ne "${CYAN}.${NC}"
            ...
            if [ "${COMMUNITY_READY}" = true ]; then
                echo -e "${GREEN}  ✓${NC} Community port is reachable"
            else
                echo -e "${YELLOW}  ⚠${NC} Community port not confirmed reachable — running user/role setup anyway"
            fi

            PY_ARGS=(--host "${COMMUNITY_HOST}" --port "${COMMUNITY_PORT}" -y)
            [[ -n "$PORT_ENV_NAME_ARG" ]] && PY_ARGS+=(--port-env-name "$PORT_ENV_NAME_ARG")
            [[ -n "$CERT_SUBDIR_ARG" ]] && PY_ARGS+=(--cert-subdir "$CERT_SUBDIR_ARG")

            echo -e "${BLUE}  →${NC} Running: ${CYAN}python3 ${USER_ROLE_SCRIPT} ${PY_ARGS[*]}${NC}"

            set +e
            python3 "$USER_ROLE_SCRIPT" "${PY_ARGS[@]}"
            USER_ROLE_RC=$?
            set -e 2>/dev/null || true

            if [ "${USER_ROLE_RC}" -eq 0 ]; then
                echo -e "${GREEN}${BOLD}✓${NC} Community users and roles configured."
            else
                echo -e "${RED}${BOLD}✗${NC} Community user/role setup failed (exit ${USER_ROLE_RC})."
                echo -e "${YELLOW}  ⚠${NC} Rerun standalone with the same arguments deploy used:"
                echo -e "      ${CYAN}python3 ${USER_ROLE_SCRIPT} ${PY_ARGS[*]}${NC}"
                echo -e "${YELLOW}  ⚠${NC} If TLS verify is the problem, add ${CYAN}--insecure${NC} to that command."
            fi
        fi
        echo -e "${CYAN}${BOLD}───────────────────────────────────────────────────────${NC}"
    fi
