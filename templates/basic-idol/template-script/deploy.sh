...
if [ $COMPOSE_EXIT_CODE -eq 0 ]; then
    ...
    if [ "$IS_UP_COMMAND" = true ]; then
        ...
        USER_ROLE_SCRIPT="$(cd "${SCRIPT_DIR}/.." && pwd)/create-users-roles.py"
        ...
        if [ ! -f "$USER_ROLE_SCRIPT" ]; then
            ...
            echo -e "${BLUE}  →${NC} PORT_BASIC_IDOL_COMMUNITY=${GREEN}${PORT_BASIC_IDOL_COMMUNITY:-}${NC}"
            ...
            if [ "${USER_ROLE_RC}" -eq 0 ]; then
                ...
                echo -e "      ${CYAN}python3 ${USER_ROLE_SCRIPT} --host ${COMMUNITY_HOST} --port ${COMMUNITY_PORT} -y --port-env-name PORT_BASIC_IDOL_COMMUNITY --cert-subdir basic-idol${NC}"
