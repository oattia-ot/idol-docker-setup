...
#   - Ownership fix on IDOL_BASE_PATH before ANY compose up
...
# Must run AFTER pre-setup.sh and BEFORE any "docker compose up"
# (including llm-deploy.sh / deploy-llm-wiki.sh).
# Do NOT wrap this in a subshell — die/return must stop the parent script.
fix_idol_base_ownership() {
    local base owner group before after

    base="${IDOL_BASE_PATH:-}"
    if [[ -z "$base" ]]; then
        die "IDOL_BASE_PATH is not set. Source pre-setup.sh or export IDOL_BASE_PATH (e.g. /home/kduser6/idol-docker-setup)."
    fi
    base="${base/#\~/$HOME}"
    if [[ ! -d "$base" ]]; then
        die "IDOL_BASE_PATH is not a directory: $base"
    fi

    owner="$(id -un)"
    group="$(id -gn)"
    command -v sudo >/dev/null || die "sudo is required to fix ownership on $base"

    before="$(stat -c '%U:%G' "$base" 2>/dev/null || echo unknown)"
    log STEP "Fixing ownership on IDOL base root: $base"
    log INFO "Target owner: ${owner}:${group}  |  current dir owner: ${before}"
    log INFO "Running: sudo chown ${owner}:${group} -R -- $base"

    if ! sudo chown "${owner}:${group}" -R -- "$base"; then
        die "Failed to chown $base → ${owner}:${group}"
    fi

    after="$(stat -c '%U:%G' "$base" 2>/dev/null || echo unknown)"
    if [[ "$after" != "${owner}:${group}" ]]; then
        die "chown returned 0 but $base is still owned by ${after} (expected ${owner}:${group})"
    fi
    log SUCCESS "Ownership set to ${owner}:${group} on $base"
}
...
# Fix host-side permissions before ANY compose up (LLM / wiki / main stack).
# Skipped on "down" — teardown does not need a recursive chown.
if ! $IS_DOWN; then
    fix_idol_base_ownership
fi
...
if $IS_UP; then
    ...
    if [[ -n "${PORT_BASIC_IDOL_COMMUNITY:-}" ]] && [[ "${PORT_BASIC_IDOL_COMMUNITY}" =~ ^[0-9]+$ ]] \
       && (( PORT_BASIC_IDOL_COMMUNITY >= 1 && PORT_BASIC_IDOL_COMMUNITY <= 65535 )); then
        export COMMUNITY_PORT="${PORT_BASIC_IDOL_COMMUNITY}"
        log INFO "COMMUNITY_PORT set from PORT_BASIC_IDOL_COMMUNITY → ${COMMUNITY_PORT}"
    else
