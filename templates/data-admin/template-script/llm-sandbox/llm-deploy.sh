...
# Return "container_id\tname" for any docker container publishing $1 on the host (empty if none)
get_docker_container_on_port() {
  local port="$1"
  command -v docker &>/dev/null || return 0
  docker ps --format '{{.ID}}\t{{.Names}}\t{{.Ports}}' 2>/dev/null \
    | awk -F'\t' -v p=":${port}->" '$3 ~ p { print $1"\t"$2 }'
}

# Return newline-separated PIDs currently listening on $1 (empty if none)
get_port_pids() {
  local port="$1"
  local pids=""
...
# Return 0 if Ollama responds on its API port, 1 otherwise.
# Works whether Ollama is running natively or inside a container, since
# either way it's reachable through the published host port.
ollama_is_healthy() {
  command -v curl &>/dev/null || return 1
  curl -fsS --max-time 3 "http://localhost:11434/api/version" &>/dev/null
}

# Called when the user declines to clear a port conflict. If Ollama is
# already up and healthy, there's nothing to deploy — skip the rest of
# llm-deploy.sh (env prep, compose, model import) and let the caller
# continue with the rest of its own setup instead of hard-aborting.
skip_if_ollama_healthy_else_abort() {
  info "Checking if Ollama is already healthy before aborting..."
  if ollama_is_healthy; then
    success "Ollama is already running and healthy on port 11434."
    info "Skipping llm-deploy.sh — continuing with setup."
    exit 0
  fi
  error "Ollama is not responding and required ports are not free. Aborting."
  exit 1
}

# ── Layer 1: scan every port for a Docker container already publishing it ──
# Docker's port forwarding (WSL2 Desktop VM, or native iptables DNAT)
# frequently has no OS-visible listening process, so this must be
# checked directly via the Docker API rather than lsof/ss/fuser.
# All conflicts are collected first so there is a single confirmation
# prompt covering every port, instead of one prompt per port.
DOCKER_CONFLICT_CID=()
DOCKER_CONFLICT_CNAME=()
DOCKER_CONFLICT_PORT=()
DOCKER_CONFLICT_LABEL=()

for entry in "${PORTS_TO_CHECK[@]}"; do
  port="${entry%%:*}"
  label="${entry#*:}"
  docker_hit=$(get_docker_container_on_port "$port")
  [[ -z "$docker_hit" ]] && continue

  warn "Port $port${label:+ ($label)} is already published by the following container(s):"
  while IFS=$'\t' read -r cid cname; do
    [[ -z "$cid" ]] && continue
    echo -e "   ${CYAN}${cid}${NC} → ${cname}"
    DOCKER_CONFLICT_CID+=("$cid")
    DOCKER_CONFLICT_CNAME+=("$cname")
    DOCKER_CONFLICT_PORT+=("$port")
    DOCKER_CONFLICT_LABEL+=("$label")
  done <<< "$docker_hit"
done

if (( ${#DOCKER_CONFLICT_CID[@]} > 0 )); then
  read -r -p "$(echo -e "${YELLOW}Stop and remove the container(s) above and continue? (y/N): ${NC}")" confirm
  if [[ "$confirm" =~ ^[Yy]$ ]]; then
    for i in "${!DOCKER_CONFLICT_CID[@]}"; do
      cid="${DOCKER_CONFLICT_CID[$i]}"
      cname="${DOCKER_CONFLICT_CNAME[$i]}"
      if docker rm -f "$cid" &>/dev/null; then
        success "Removed container $cname ($cid)"
      else
        error "Failed to remove container $cname ($cid)"
        exit 1
      fi
    done
    sleep 1
...
# ── Layer 2: fall back to OS-level socket check for non-Docker processes ──
# Same idea: collect every port's conflicting PIDs first, then ask once.
PID_CONFLICT_PID=()
PID_CONFLICT_PORT=()
PID_CONFLICT_LABEL=()
...
for entry in "${PORTS_TO_CHECK[@]}"; do
  ...
  if [[ -z "$pids" ]]; then
    success "Port $port${label:+ ($label)} is free"
    continue
  fi

  warn "Port $port${label:+ ($label)} is already in use by the following process(es):"
  while IFS= read -r pid; do
    [[ -z "$pid" ]] && continue
    pname=$(ps -p "$pid" -o comm= 2>/dev/null || echo "unknown")
    echo -e "   ${CYAN}PID ${pid}${NC} → ${pname}"
    PID_CONFLICT_PID+=("$pid")
    PID_CONFLICT_PORT+=("$port")
    PID_CONFLICT_LABEL+=("$label")
  done <<< "$pids"
done

if (( ${#PID_CONFLICT_PID[@]} > 0 )); then
  read -r -p "$(echo -e "${YELLOW}Kill the process(es) above and continue? (y/N): ${NC}")" confirm
  if [[ "$confirm" =~ ^[Yy]$ ]]; then
    for i in "${!PID_CONFLICT_PID[@]}"; do
      pid="${PID_CONFLICT_PID[$i]}"
      port="${PID_CONFLICT_PORT[$i]}"
      if kill -9 "$pid" 2>/dev/null; then
        success "Killed process $pid on port $port"
      else
        error "Failed to kill process $pid (you may need sudo, or it may already be gone)"
        exit 1
      fi
    done
    sleep 1
    # Verify every affected port actually freed up
    for port in $(printf '%s\n' "${PID_CONFLICT_PORT[@]}" | sort -u); do
      if [[ -n "$(get_port_pids "$port")" ]]; then
        error "Port $port is still in use after killing the process(es). Aborting."
        exit 1
      fi
