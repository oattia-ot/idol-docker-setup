...
- [Installation Guide](#installation-guide)
  ...
  - [Phase 1 — WSL Environment Setup (Windows only)](#phase-1--wsl-environment-setup-windows-only)
  - [Phase 2 — Clone the repository](#phase-2--clone-the-repository)
  - [Phase 3 — Install host prerequisites](#phase-3--install-host-prerequisites)
  - [Phase 4 — Download IDOL License Server](#phase-4--download-idol-license-server)
  - [Phase 5 — Deploy and configure the Setup UI](#phase-5--deploy-and-configure-the-setup-ui)
...
- [Utilities Tools](#utilities-tools)
  - [Airgap Images](#airgap-images)
  - [Build IDOL Images](#build-idol-images)
  - [Config Placeholders](#config-placeholders)
  - [Docker Setup](#docker-setup)
  - [Folder Compare](#folder-compare)
  - [Generate SSL Certs](#generate-ssl-certs)
  - [Harbor Setup](#harbor-setup)
  - [LDAP Setup](#ldap-setup)
  - [Mailbox Setup](#mailbox-setup)
  - [Merge PDFs](#merge-pdfs)
  - [Minikube Setup](#minikube-setup)
  - [NiFi Registry Setup](#nifi-registry-setup)
  - [NiFi Utilities](#nifi-utilities)
  - [UI Config](#ui-config)
...
|---|---|---|
| **Operating System** | Ubuntu 24.04 LTS **or macOS** (Intel / Apple silicon) | Scripts auto-detect the host OS (`module/os-compat.sh`) |
...
Host scripts detect Ubuntu vs macOS (`IDOL_HOST_OS` / `IDOL_OS_FAMILY`) and branch package installs (`apt` vs Homebrew), Docker install (Engine + systemd vs Docker Desktop), and GNU/BSD command differences (`sed -i`, `stat`, `hostname -I`, …).

### Required Software

The following packages are installed in **Phase 3** (after you clone the repo). Ubuntu uses `apt`; macOS uses Homebrew + Docker Desktop:

- Docker Engine (≥ 24.04) + Docker Compose (≥ 2.0) — **or Docker Desktop on macOS**
...
| Order | Phase | What you do |
|---|---|---|
| 1 | [Phase 1](#phase-1--wsl-environment-setup-windows-only) | Windows only — create a WSL Ubuntu distro. Skip on native Ubuntu or macOS. |
| 2 | [Phase 2](#phase-2--clone-the-repository) | `git clone` this repository. |
| 3 | [Phase 3](#phase-3--install-host-prerequisites) | Install Docker, Java, OpenSSL, and `jq` using the scripts now in the clone. |
| 4 | [Phase 4](#phase-4--download-idol-license-server) | Download License Server + `licensekey.dat` and place them on the host. |
| 5 | [Phase 5](#phase-5--deploy-and-configure-the-setup-ui) | Start the Setup Manager UI and fill in the wizard. |
| 6 | [Phase 6](#phase-6--deploy-idol-services) | Run the generated deploy command. |

Stay in this order. The prerequisite scripts live *inside* the repo, so clone (Phase 2) always comes before installing tools (Phase 3).

---

### Phase 1 — WSL Environment Setup (Windows only)

<details>
<summary>⚠️ <strong>Optional</strong> — Skip this phase if you are on native Ubuntu or macOS and continue to Phase 2 (clone).</summary>
...
> Ubuntu/WSL: open a guest terminal as your regular user (e.g. `demo`).  
> macOS: use Terminal.app / iTerm in the folder where you want the project.
...
You must be in `idol-docker-setup/` for every command in Phases 3 and 5.

---

### Phase 3 — Install host prerequisites

Run **one** of these from the repository root (`idol-docker-setup/`).

```bash
# Auto-detect Ubuntu vs macOS (preferred)
./utilities/docker-setup/install-prereqs.sh

# Ubuntu / Debian only (needs sudo)
sudo ./utilities/docker-setup/install-ubuntu-prereqs.sh

# macOS only (Homebrew + Docker Desktop — do not use apt)
...
| Host | Installer | Runtime |
|---|---|---|
| Ubuntu 24.04 / Debian | `install-ubuntu-prereqs.sh` | Docker Engine + systemd |
| macOS (Intel / Apple silicon) | `install-macos-prereqs.sh` | Docker Desktop |

Both scripts install Docker / Compose, OpenJDK 21, OpenSSL, jq, git, and curl. Pass `--with-optional` for helm, kubectl, minikube, and ghostscript. See `utilities/docker-setup/README.md` and `OS-COMPAT.md`.

**macOS — start Docker Desktop and confirm the engine** (required before `docker compose` works):
...
`docker info` must show a **Server** section. If it errors with *Cannot connect to the Docker daemon*, Docker Desktop is still starting — wait and retry. On Apple silicon enable Rosetta for amd64 images (Settings → General).
...
<details>
<summary>⚙️ <strong>Optional — Ubuntu / WSL manual install</strong> — only if you cannot use <code>install-ubuntu-prereqs.sh</code> above. Stay in the repo root and use these <code>apt</code> commands instead of the script.</summary>

#### 2.1 — Install Docker Engine (Ubuntu manual)
...
### Phase 4 — Download IDOL License Server

> Complete this step **before** opening the Setup Manager UI. The license server binary and key file must be in place so that their paths can be entered in the Setup UI (Phase 5 — License step).
...
> **Note:** The paths set here (`~/LicenseServer_25.x.x_LINUX_X86_64` and `~/licensekey.dat`) are the values you will enter in **Phase 5 — License Server** in the Setup UI wizard.

---

---

### Phase 5 — Deploy and configure the Setup UI
...
**macOS note:** port 5000 is often taken by AirPlay Receiver. If deploy fails with `bind: address already in use`, turn off **System Settings → General → AirDrop & Handoff → AirPlay Receiver**, or publish the UI on `5001:5000` in `utilities/ui-config/backend/ui-docker-compose.yml`.
...
For environments without direct internet access, use the airgap utility to bundle and load the Docker images required by this deployment.

- **Script** — `utilities/airgap-images/airgap-images.sh`
- **Docs** — `utilities/airgap-images/AIRGAP-IMAGES.md`

Refer to `AIRGAP-IMAGES.md` for full usage instructions, supported options, and details on transferring the resulting image bundle to a disconnected host.

### Build IDOL Images

Build custom IDOL Docker images locally.

- **Script** — `utilities/build-idol-images/build-docker-image.sh`
- **Docs** — `utilities/build-idol-images/build-docker-image.md`

### Config Placeholders

Replace placeholder tokens in configuration files with environment-specific values.

- **Script** — `utilities/config-placeholders/placeholders-replacement.sh`

### Docker Setup

Install and configure Docker on the host.

- **Docs** — `utilities/docker-setup/README.md`

### Folder Compare

Compare two folders and copy over differing or missing files.

- **Script** — `utilities/folder-compare/comper-and-copy.sh`

### Generate SSL Certs

Generate the SSL certificates used by IDOL services and the Setup Manager UI.

- **Script** — `utilities/generate-ssl-certs/generate-ssl.sh`

### Harbor Setup

Install and configure a Harbor container registry.

- **Docs** — `utilities/harbor-setup/install-harbor.md`
- **Script** — `utilities/harbor-setup/install-harbor.sh`

### LDAP Setup

Deploy and configure LDAP for authentication.

- **Docs** — `utilities/ldap-setup/ldap-setup-guide.md`
- **Script** — `utilities/ldap-setup/ldap-deploy.sh`

### Mailbox Setup

Set up a mailbox/mail server for the deployment.

- **Docs** — `utilities/mailbox-setup/README.md`

### Merge PDFs

Merge multiple PDF files into one.

- **Script** — `utilities/merge-pdfs/merge-all-pdfs.sh`

### Minikube Setup

Deploy a local Minikube cluster.

- **Script** — `utilities/minikube-setup/deploy-minikube.sh`
- **Docs** — `utilities/minikube-setup/setup-minikube.md`

### NiFi Registry Setup

Deploy the NiFi Registry with Git integration.

- **Script** — `utilities/nifi-registry-setup/deploy-nifi-registry-with-git.sh`

### NiFi Utilities

Additional NiFi-related helper scripts and resources.

- **Folder** — `utilities/nifi-utilities`

### UI Config

Deploy the Setup Manager UI.

- **Script** — `utilities/ui-config/deploy-setup-manager-ui.sh`
...
Recreate a Single Container (--force-recreate with stacked compose files)
