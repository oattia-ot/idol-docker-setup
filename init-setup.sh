#!/usr/bin/env bash
# OpenText IDOL on Ubuntu 24.04 or macOS
# Host OS is auto-detected (ubuntu / macos) via module/os-compat.sh
set -euo pipefail
...
if [ -f "$SCRIPT_DIR/module/os-compat.sh" ]; then
    # shellcheck source=/dev/null
    source "$SCRIPT_DIR/module/os-compat.sh"
fi
...
cleanup_dangling_docker_volumes() {
    ...
    # Final check to confirm no dangling volumes remain - Countdown
...
##################################
## Copy Shared Community Scripts
##################################
copy_shared_community_scripts() {
    export CALLING_SCRIPT="${CYAN}${EXE_SCRIPT_NAME%.*} [copy_shared_community_scripts]${ORANGE}"

    echo ''
    log "${CALLING_SCRIPT} ${YELLOW}Copying shared Community users/roles scripts into idol-containers-toolkit...${NC}"

    local TEMPLATES_DIR="$IDOL_BASE_PATH/templates"
    local DEST_DIR="$IDOL_BASE_PATH/idol-containers-toolkit"

    local FILES=("create-users-roles.py" "create-users-roles.md")
    local copied=0
    local failed=0

    if [ ! -d "$DEST_DIR" ]; then
        log "${CALLING_SCRIPT} ⚠ Destination directory does not exist yet: ${ORANGE}${DEST_DIR}${NC}"
        echo -e "${YELLOW}⚠ ${DEST_DIR} not found — skipping copy of shared Community scripts.${NC}"
        return 0
    fi
    ...
    for f in "${FILES[@]}"; do
        ...
        if [ ! -f "$src" ]; then
            log "${CALLING_SCRIPT} ❌ Source file missing: ${ORANGE}${src}${NC}"
            echo -e "${RED}❌ Missing template: ${ORANGE}${src}${NC}"
            failed=$((failed + 1))
            continue
        fi

        if cp -f "$src" "$dest" && [ -s "$dest" ]; then
            log "${CALLING_SCRIPT} ✅ Copied ${ORANGE}${src}${NC} → ${ORANGE}${dest}${NC}"
            echo -e "${GREEN}✅ Copied ${ORANGE}${f}${GREEN} → ${ORANGE}${DEST_DIR}${NC}"
            copied=$((copied + 1))
        else
            log "${CALLING_SCRIPT} ❌ Failed to copy ${ORANGE}${src}${NC} → ${ORANGE}${dest}${NC}"
            echo -e "${RED}❌ Failed to copy ${ORANGE}${f}${RED} to ${ORANGE}${DEST_DIR}${NC}"
            failed=$((failed + 1))
        fi
    done

    if [ "$failed" -gt 0 ]; then
        log "${CALLING_SCRIPT} ⚠ Completed with ${failed} failure(s), ${copied} file(s) copied."
        echo -e "${YELLOW}⚠ Shared Community script copy completed with ${failed} failure(s).${NC}"
        return 1
    fi

    log "${CALLING_SCRIPT} ✅ All shared Community scripts copied successfully (${copied} file(s))."
    echo -e "${GREEN}✅ Shared Community scripts (create-users-roles.py / .md) are up to date in ${ORANGE}${DEST_DIR}${NC}"
}
...
run_ollama_resource_estimator() {
    ...
    if [ "${IDOL_LLM_ENABLE_APIKEY:-}" = "TRUE" ]; then
        ...
        if [ -f "$ANSWERSERVER_CFG" ]; then
            idol_sed_inplace 's|^//[[:space:]]*2[[:space:]]*=[[:space:]]*Grok|2=Grok|' "$ANSWERSERVER_CFG"
