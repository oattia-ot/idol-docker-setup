...
if [ -f "$_SCRIPT_DIR/module/os-compat.sh" ]; then
    # shellcheck source=/dev/null
    . "$_SCRIPT_DIR/module/os-compat.sh"
    idol_os_banner
fi
...
truncate_file() {
  ...
  elif : > "$target" 2>/dev/null || sudo sh -c ": > \"$target\"" 2>/dev/null; then
    success "$label (truncated)"
    return
  fi
...
remove "idol-containers-toolkit/basic-idol/deploy.sh"                      "basic-idol deploy.sh"
remove "idol-containers-toolkit/data-admin/deploy.sh"                      "data-admin deploy.sh"
remove "idol-containers-toolkit/rich-media/deploy.sh"                      "rich-media deploy.sh"
...
remove "env/.[!.]*"                                                        "env/.[!.]*"
remove "logs/*"                                                            "logs/*"
remove "persistent-data/dataadmin_community/cfg/logs/*"                    "persistent-data/dataadmin_community/cfg/logs/*"
...
if command -v idol_sed_inplace >/dev/null 2>&1; then
  idol_sed_inplace 's/^API_KEY[[:space:]]*=.*$/API_KEY = "LLM-API-KEY-PLACEHOLDER"  # Replace with your actual API key in production/' ./persistent-data/answerserver/rag/grok/grok4.py
