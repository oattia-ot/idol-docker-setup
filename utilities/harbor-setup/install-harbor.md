...
Host scripts detect **Ubuntu/Linux** vs **macOS** (`module/os-compat.sh`). Prerequisites (`curl`, `wget`, `python3`, `openssl`, Docker) are installed with `apt` on Ubuntu and Homebrew / Docker Desktop on macOS. `systemctl` Docker restarts are skipped on macOS — restart Docker Desktop instead.
