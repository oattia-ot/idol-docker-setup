...
|--------|---------------------------|-----------------------------|
| 5000 (default, configurable) | Config Manager UI | Main web interface. Override with `--port` / `SETUP_UI_PORT`. |
...
The script asks **What is the UI port? [5000]**. Press Enter for 5000, or type another host port (for example `5001` on macOS if AirPlay already uses 5000).

```bash
# Skip the prompt
./deploy-setup-manager-ui.sh --deploy --port 5001
SETUP_UI_PORT=5001 ./deploy-setup-manager-ui.sh --deploy
```
...
| Port already in use                  | Another process or container bound  | Script asks to kill the listener; or `--port <free>` |
...
| `Is a directory: '/setup-scripts/pre-setup.sh'` | Docker bind-mounted a missing host file as a directory | `rm -rf "$IDOL_BASE_PATH/pre-setup.sh" && touch "$IDOL_BASE_PATH/pre-setup.sh"` then `./deploy-setup-manager-ui.sh --deploy` |
