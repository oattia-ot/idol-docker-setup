...
export function initDashboardPage() {
  ...
  function updateConfigLinks() {
    ...
    // Same host as this dashboard (the common case) → use relative hrefs so
    // the link always follows whatever port is in the address bar right now,
    // instead of a hardcoded port that goes stale the moment the UI is
    // published on something other than 5000. Never read a cached
    // "idol_setupUiPort" style value from localStorage for this — the
    // address bar (window.location) is the only source of truth.
    if (host === 'localhost') {
      if (idolBtn) idolBtn.href = '/config-idol';
      if (nifiBtn) nifiBtn.href = '/config-nifi';
      return;
    }

    // Extra IP SANs override → different host, but still same port as this page.
    const currentPort = window.location.port ? `:${window.location.port}` : '';
    if (idolBtn) idolBtn.href = `${window.location.protocol}//${host}${currentPort}/config-idol`;
    if (nifiBtn) nifiBtn.href = `${window.location.protocol}//${host}${currentPort}/config-nifi`;
  }
