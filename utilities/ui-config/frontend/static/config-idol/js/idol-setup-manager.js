...
function updateHomeBtnHref() {
  ...
  // Always follow window.location.port (the port actually published for this
  // page) rather than a hardcoded 5000 — the UI can be published on any port.
  const currentPort = window.location.port ? `:${window.location.port}` : '';
  if (ipToggle && ipToggle.checked && !isDefault && extraIps.length > 0) {
    homeBtn.href = `${window.location.protocol}//${extraIps[0]}${currentPort}/`;
  } else {
    homeBtn.href = '/';
  }
}
...
const DA_PORTS = [
  ...
  { id:'dataadminContentPort', statusId:'dataadminContentPortStatus', warnId:'dataadminContentPortWarning', dupeId:'dataadminContentPortDupeWarning', label:'dataadmin Extractor Content', envKey:'PORT_DATA_ADMIN_CONTENT', def:9103, range:true, aciId:'dataadminContentAci', idxId:'dataadminContentIdx', svcId:'dataadminContentSvc' },
  ...
  { id:'dataadminAgentPort', statusId:'dataadminAgentPortStatus', warnId:'dataadminAgentPortWarning', dupeId:'dataadminAgentPortDupeWarning', label:'dataadmin Extractor AgentStore', envKey:'PORT_DATA_ADMIN_AGENTSTORE', def:12310, range:true, aciId:'dataadminAgentAci', idxId:'dataadminAgentIdx', svcId:'dataadminAgentSvc' },
  ...
  { id:'daCategoryPort', statusId:'daCategoryPortStatus', warnId:'daCategoryPortWarning', dupeId:'daCategoryPortDupeWarning', label:'DA Category', envKey:'PORT_DATA_ADMIN_CATEGORY', def:9060, range:true, aciId:'daCategoryAci', idxId:'daCategoryIdx', svcId:'daCategorySvc' },
  { id:'daCatAgentStorePort', statusId:'daCatAgentStorePortStatus', warnId:'daCatAgentStorePortWarning', dupeId:'daCatAgentStorePortDupeWarning', label:'DA Cat. AgentStore', envKey:'PORT_DATA_ADMIN_CATEGORISATION_AGENTSTORE', def:9070, range:true, aciId:'daCatAgentStoreAci', idxId:'daCatAgentStoreIdx', svcId:'daCatAgentStoreSvc' }
];
...
const ENV_VAR_ORDER = [
  ...
  'IDOL_PRESERVE_ANSWERSERVER_PATH','IDOL_PRESERVE_CATEGORY_PATH','IDOL_PRESERVE_CATEGORISATION_AGENTSTORE_PATH','IDOL_PRESERVE_DATAADMIN_CATEGORY_PATH','IDOL_PRESERVE_DATAADMIN_CATEGORISATION_AGENTSTORE_PATH',
  ...
  'IDOL_PRESERVE_LICENSESERVER_CFG_PATH','IDOL_PRESERVE_DATAADMIN_AGENTSTORE_PATH',
  'IDOL_PRESERVE_DATAADMIN_CONTENT_PATH','IDOL_PRESERVE_PATH','IDOL_PRESERVE_QMS_AGENTSTORE_PATH','IDOL_PRESERVE_QMS_PATH',
  ...
  'PORT_DATA_ADMIN_CATEGORISATION_AGENTSTORE','PORT_DATA_ADMIN_CATEGORISATION_AGENTSTORE_INDEX','PORT_DATA_ADMIN_CATEGORISATION_AGENTSTORE_SERVICE','PORT_DATA_ADMIN_CATEGORISATION_AGENTSTORE_QUERY',
  'PORT_DATA_ADMIN_CATEGORY','PORT_DATA_ADMIN_CATEGORY_INDEX','PORT_DATA_ADMIN_CATEGORY_SERVICE',
  'PORT_DATA_ADMIN_COMMUNITY','PORT_DATA_ADMIN_COMMUNITY_INDEX','PORT_DATA_ADMIN_COMMUNITY_SERVICE',
  'PORT_DATA_ADMIN_CONTENT','PORT_DATA_ADMIN_CONTENT_INDEX','PORT_DATA_ADMIN_CONTENT_SERVICE',
  'PORT_DATA_ADMIN_AGENTSTORE','PORT_DATA_ADMIN_AGENTSTORE_INDEX','PORT_DATA_ADMIN_AGENTSTORE_SERVICE',
...
/* ====================== 6b. DEPLOYMENT TYPE EXCLUSIVITY ======================
   "Basic IDOL" and "Data Admin IDOL" represent two different base deployments
   and can never both be active at the same time. "Rich Media" is an add-on and
   is NOT part of this exclusivity — it can be combined with either one. */
function enforceDeploymentTypeExclusivity(changedEl) {
  const basicCb = document.getElementById('deployTypeBasic');
  const dataAdminCb = document.getElementById('deployTypeDataAdmin');
  if (!basicCb || !dataAdminCb || !changedEl) return;

  if (changedEl.id === 'deployTypeBasic' && basicCb.checked) {
    dataAdminCb.checked = false;
  } else if (changedEl.id === 'deployTypeDataAdmin' && dataAdminCb.checked) {
...
function setupIdolToggleConditionalFields() {
  ...
  if (preserve === 'TRUE' && storSub && subList) {
    ...
    subList.innerHTML = ['content','find','community','agentstore','category','categorisation_agentstore','view','dataadmin_content','dataadmin_agentstore','dataadmin','dataadmin_community','dataadmin_viewserver','dataadmin_statsserver','qms','qms_agentstore','answerserver','answerbank_agentstore','licenseserver','content/cfg'].map(d => `<li style="font-family:var(--font-mono);font-size:11px;">${pp}/${d}</li>`).join('');
...
function resetPortsToDefaults() {
  const defaults = {
    ...
    communityPort: "9033",
    viewServerPort: "9083",
    dataadminContentPort: "9103",
    answerServerPort: "12000",
    answerBankAgentPort: "12200",
    dataadminAgentPort: "12310",
    qmsPort: "16000",
    qmsAgentPort: "20050",
...
function llmLoadDefaultModels() {
  ...
  fetch('/load-models/default-models.json?t=' + Date.now())
    .then(async r => {
      if (!r.ok) {
        // Surface the backend's actual diagnostic (e.g. "mount is a
        // directory", "not valid JSON") instead of just the HTTP status.
        let detail = `HTTP ${r.status}`;
        try {
          const body = await r.json();
          if (body && (body.detail || body.error)) detail = body.detail || body.error;
        } catch (_) { /* body wasn't JSON, keep the HTTP status message */ }
    ...
    .then(data => {
      ...
      llmIngestJson(data);
      const total = _llmData.supported.length + _llmData.custom_required.length;
      if (statusDiv) statusDiv.innerHTML = `<span style="color:#10b981">✅ Loaded ${total} models</span>`;
    })
    .catch(err => {
      console.error('❌ Failed to load default-models.json:', err);
      if (statusDiv) {
        statusDiv.innerHTML = `<span style="color:#ef4444">⚠️ Could not load models: ${err.message}<br>Using demo fallback...</span>`;
      }
...
function llmIngestJson(data) {
  ...
  // Include BOTH buckets — previously this only mapped `supported`, so any
  // model listed under `custom_required` in default-models.json could be
  // downloaded/selected in the picker grid but could never appear as an
  // option in the "Default LLM for Answer Server" dropdown below.
  llmSupportedModels = [..._llmData.supported, ..._llmData.custom_required].map(m => ({
    id: m.name,
    name: m.name
  }));
...
function llmSelectCardForAnswerServer(modelName) {
  ...
  // Look across BOTH buckets — modelName may belong to custom_required now
  // that llmSupportedModels includes it too. Index must line up with the
  // _idx values used by llmRenderPicker/llmGetModelByIdx (supported first,
  // then custom_required).
  const allModels = [..._llmData.supported, ..._llmData.custom_required];
  const idx = allModels.findIndex(m => m.name === modelName);
  if (idx === -1) return;
...
function setupIdolGetEnv() {
  ...
  // Data Admin no longer has its own version selector — it always tracks
  // whatever IDOL Server version was picked for Basic IDOL.
  env.IDOL_DATA_ADMIN_VERSION  = dataAdminOn ? env.IDOL_SERVER_VERSION : '';
  ...
  const daMap = {
    ...
    PORT_DATA_ADMIN_CATEGORY:              g('daCategoryPort')      || '9060',
    PORT_DATA_ADMIN_CATEGORISATION_AGENTSTORE: g('daCatAgentStorePort') || '9070',
  };
  ...
  const daCatAgentBase = +(g('daCatAgentStorePort') || '9070');
  env.PORT_DATA_ADMIN_CATEGORISATION_AGENTSTORE_QUERY = String(daCatAgentBase + 3);
  ...
  const subDirs = {
    ...
    IDOL_PRESERVE_DATAADMIN_CATEGORY_PATH:                     `${pp}/dataadmin_category`,
    IDOL_PRESERVE_DATAADMIN_CATEGORISATION_AGENTSTORE_PATH:    `${pp}/dataadmin_categorisation_agentstore`,    
    IDOL_PRESERVE_DATAADMIN_CONTENT_PATH:            `${pp}/dataadmin_content`,
    IDOL_PRESERVE_DATAADMIN_AGENTSTORE_PATH:         `${pp}/dataadmin_agentstore`,
...
function setupIdolGeneratePreSetupScript() {
  ...
  ENV_VAR_ORDER.forEach(key => {
    ...
    if (key === 'IDOL_DATA_ADMIN_INSTALL') {
      ...
      // Data Admin always tracks the Basic IDOL server version now.
  ...
  if (env.IS_IDOL_PRESERVE === 'TRUE' && env.IDOL_PRESERVE_PATH) {
    const dirs = [
      ...
      env.IDOL_PRESERVE_DATAADMIN_CATEGORY_PATH,
      env.IDOL_PRESERVE_DATAADMIN_CATEGORISATION_AGENTSTORE_PATH,
      env.IDOL_PRESERVE_DATAADMIN_CONTENT_PATH,
      env.IDOL_PRESERVE_DATAADMIN_AGENTSTORE_PATH,
...
async function setupIdolSaveScriptToServer() {
  ...
  try {
    const r = await fetch('/config-idol/save-script', {
      method:'POST',
      headers:{ 'Content-Type':'application/json' },
      body: JSON.stringify({ file_path:'/idol-base/pre-setup.sh', script_content:sc.textContent })
    });
...
function updateInfoSectionUrls() {
  ...
  const p = {
    ...
    // NOTE: the Ports Config field for this service is "passageContentPort" — a prior
    // version of this code read a non-existent "dataadminContentPort" id and silently
    // fell back to the default every time, ignoring whatever the user configured.
    daContent: port('passageContentPort', 9103),
    daCategory: port('daCategoryPort', 9060),
    daCatAgentStore: port('daCatAgentStorePort', 9070),

    // Answer Server / QMS / Stats
    answerServer: port('answerServerPort', 12000),
    answerBank: port('answerBankAgentPort', 12200),
    dataadminAgent: port('dataadminAgentPort', 12310),
    qms: port('qmsPort', 16000),
    qmsAgent: port('qmsAgentPort', 20050),
    stats: port('statsServerPort', 19870),

    // Rich Media (ACI / Svc / Srv-SSL / Svc-SSL — a contiguous 4-port range starting at rmMediaServerPort)
    rmMedia: port('rmMediaServerPort', 14000),
    rmMediaSrvSSL: port('rmMediaServerPort', 14000) + 2,
    rmMediaSvcSSL: port('rmMediaServerPort', 14000) + 3,
    rmPlaylist: port('rmMediaServerPlaylistPort', 24000),

    // IDOL UIs & Proxies
    findDaUi: port('findUiPort', 8440) + 1,
  ...
  // ====================== MAIN SERVICE LINKS ======================
  // Single source of truth for every Quick Access link: [linkId, protocol, portKey, path, copyId].
  // Adding a new service only ever means adding one row here (plus its HTML row) —
  // no more hunting through dozens of near-identical setLink() calls to wire up a URL.
  const SERVICE_LINKS = [
    // NiFi
    ['dyn-nifi-card-link',            'https', 'nifi',            '/nifi',              'dyn-nifi-card-copy'],
    ['dyn-nifi-registry-link',        'http',  'nifiRegistry',    '/nifi-registry',     'dyn-nifi-registry-copy'],

    // Basic IDOL
    ['dyn-category-link',             'https', 'biCategory',      '/a=admin',           'dyn-category-copy'],
    ['dyn-community-link',            'https', 'biCommunity',     '/a=admin',           'dyn-community-copy'],
    ['dyn-content-link',              'https', 'biContent',       '/a=admin',           'dyn-content-copy'],
    ['dyn-agentstore-link',           'https', 'biAgentStore',    '/a=admin',           'dyn-agentstore-copy'],
    ['dyn-catagentstore-link',        'https', 'biCatAgentStore', '/a=admin',           'dyn-catagentstore-copy'],
    ['dyn-view-link',                 'https', 'biView',          '/a=admin',           'dyn-view-copy'],

    // Data Admin Classic
    ['dyn-da-community-link',         'https', 'daCommunity',     '/a=admin',           'dyn-da-community-copy'],
    ['dyn-da-view-link',              'https', 'daView',          '/a=admin',           'dyn-da-view-copy'],
    ['dyn-da-content-link',           'https', 'daContent',       '/a=admin',           'dyn-da-content-copy'],
    ['dyn-da-category-link',          'https', 'daCategory',      '/a=admin',           'dyn-da-category-copy'],
    ['dyn-da-catagentstore-link',     'https', 'daCatAgentStore', '/a=admin',           'dyn-da-catagentstore-copy'],

    // REST GetStatus links
    ['dyn-rest-comm-status',          'https', 'daCommunity',     '/action=GetStatus',  'dyn-rest-comm-status-copy'],
    ['dyn-rest-cont-status',          'https', 'daContent',       '/action=GetStatus',  'dyn-rest-cont-status-copy'],
    ['dyn-rest-bi-comm-status',       'https', 'biCommunity',     '/action=GetStatus',  'dyn-rest-bi-comm-status-copy'],
    ['dyn-rest-comm-status-hk',       'https', 'daCommunity',     '/action=GetStatus',  'dyn-rest-comm-status-hk-copy'],
    ['dyn-rest-cont-status-hk',       'https', 'daContent',       '/action=GetStatus',  'dyn-rest-cont-status-hk-copy'],
    ['dyn-rest-agent-status',         'https', 'biAgentStore',    '/action=GetStatus',  'dyn-rest-agent-status-copy'],

    // Answer Server / QMS / Stats
    ['dyn-answerserver-link',         'https', 'answerServer',    '/a=admin',           'dyn-answerserver-copy'],
    ['dyn-answerbank-link',           'https', 'answerBank',      '/a=admin',           'dyn-answerbank-copy'],
    ['dyn-dataadminagent-link',       'https', 'dataadminAgent',  '/a=admin',           'dyn-dataadminagent-copy'],
    ['dyn-qms-link',                  'https', 'qms',             '/a=admin',           'dyn-qms-copy'],
    ['dyn-qmsagent-link',             'https', 'qmsAgent',        '/a=admin',           'dyn-qmsagent-copy'],
    ['dyn-stats-link',                'https', 'stats',           '/a=admin',           'dyn-stats-copy'],

    // Rich Media
    ['dyn-rm-media-admin-link',       'https', 'rmMedia',         '/a=admin',           'dyn-rm-media-admin-copy'],
    ['dyn-rm-playlist-admin-link',    'https', 'rmPlaylist',      '/a=admin',           'dyn-rm-playlist-admin-copy'],
    ['dyn-rm-playlist-link',          'https', 'rmPlaylist',      '',                   'dyn-rm-playlist-copy'],
    ['dyn-rm-media-app-link',         'http',  'mediaServerUi',   '',                   'dyn-rm-media-app-copy'],
    ['dyn-rm-mediaserver-gui-link',   'https', 'rmMedia',         '/a=gui',             'dyn-rm-mediaserver-gui-copy'],

    // IDOL UIs & Proxies
    ['dyn-find-da-link',              'https', 'findDaUi',        '',                   'dyn-find-da-copy'],
    ['dyn-dataadmin-link',            'https', 'dataAdminUi',     '',                   'dyn-dataadmin-ui-copy'],

    // External Tools
    ['dyn-ollama-link',               'http',  'ollama',          '',                   'dyn-ollama-copy'],
    ['dyn-llmwiki-link',              'https', 'llmWiki',         '',                   'dyn-llmwiki-copy'],
    ['dyn-ldap-link',                 'http',  'ldap',            '',                   'dyn-ldap-copy'],
    ['dyn-harbor-docker-link',        'https', 'harborDocker',    '',                   'dyn-harbor-docker-copy'],
    ['dyn-harbor-minikube-link',      'https', 'harborMinikube',  '',                   'dyn-harbor-minikube-copy'],

    // Connector
    ['dyn-connector-version-link',    'https', 'connector',       '/action=GetVersion', 'dyn-connector-version-copy'],
    ['dyn-connector-status-link',     'https', 'connector',       '/action=GetStatus',  'dyn-connector-status-copy']
  ];

  SERVICE_LINKS.forEach(([linkId, proto, portKey, path, copyId]) => {
    const portNum = p[portKey];
    if (portNum === undefined) {
      console.warn(`[updateInfoSectionUrls] Unknown port key "${portKey}" for ${linkId} — skipped.`);
      return;
    }
    setLink(linkId, `${proto}://${HOST}:${portNum}${path}`, copyId);
  });

  // Media Server SSL ports are raw ACI/service TLS endpoints, not admin HTTP UIs —
  // shown as copyable "host:port" text rather than a clickable link.
  setText('dyn-rm-media-srvssl-text', `${HOST}:${p.rmMediaSrvSSL} (Srv SSL)`);
  setText('dyn-rm-media-svcssl-text', `${HOST}:${p.rmMediaSvcSSL} (Svc SSL)`);
  const rmSslCopyBtn = document.getElementById('dyn-rm-media-srvssl-copy');
  if (rmSslCopyBtn) rmSslCopyBtn.setAttribute('data-copy', `Srv SSL ${HOST}:${p.rmMediaSrvSSL} / Svc SSL ${HOST}:${p.rmMediaSvcSSL}`);
  ...
  const grlMap = {
    ...
    // NOTE: key must match the data-grl="passagecontent-da" attribute used in the HTML —
    // a prior mismatched key ("dataadmincontent-da") meant these a=grl pills never resolved.
    'passagecontent-da': { proto: 'https', portKey: 'daContent',       path: '/a=grl' },
    'category-da':       { proto: 'https', portKey: 'daCategory',      path: '/a=grl' },
    'catagentstore-da':  { proto: 'https', portKey: 'daCatAgentStore', path: '/a=grl' },

    'answerserver':      { proto: 'https', portKey: 'answerServer',    path: '/a=grl' },
    'answerbank':        { proto: 'https', portKey: 'answerBank',      path: '/a=grl' },
    'dataadminagent':      { proto: 'https', portKey: 'dataadminAgent',    path: '/a=grl' },
  ...
  // URLs just changed (host/IP toggle, FQDN edit, port edit) — re-validate reachability
  // after a short debounce so we don't re-fire ~40 checks on every keystroke.
  scheduleQuickAccessValidation();
}

/* ====================== 13b. SERVICE VALIDATION (Quick Access health) ======================
   Reuses the existing generic /config-idol/check-nifi endpoint (despite its name it just does
   an HTTP GET against whatever URL it's given and reports reachable=true/false), so every
   Quick Access link can be validated without any new backend code.

   Deployment-type gating: a card (or, for the mixed Housekeeping card, an individual row) is
   tagged with data-qa-group="…". A group is only actually validated when its matching
   Deployment Type checkbox is checked — there's no point pinging Data Admin ports when the
   selected deployment is Basic IDOL only, they were never going to be running. NiFi, IDOL UIs,
   External Tools and the Connector aren't tied to a specific deployment type, so they're
   always validated. Groups that are gated out show a dim "inactive" dot instead of red, so
   it's clear they weren't skipped because they failed — they're just not part of this
   deployment.
*/
const QA_VALIDATION_TIMEOUT_S = 4;
const QA_VALIDATION_CONCURRENCY = 6;
let qaValidationToken = 0;      // guards against a stale run overwriting a newer one
let qaValidationDebounceTimer = null;

function scheduleQuickAccessValidation() {
  clearTimeout(qaValidationDebounceTimer);
  qaValidationDebounceTimer = setTimeout(validateQuickAccessServices, 900);
}

// Is this data-qa-group actually part of the currently-selected Deployment Type(s)?
function qaGroupActive(groupKey) {
  switch (groupKey) {
    case 'basic-idol':
      return document.getElementById('deployTypeBasic')?.checked || false;
    case 'data-admin-basic':
    case 'data-admin-advance':
      return document.getElementById('deployTypeDataAdmin')?.checked || false;
    case 'rich-media':
      return document.getElementById('deployTypeRichMedia')?.checked || false;
    // The "Find" UI (IDOL UIs card) is relevant whenever either base deployment is active —
    // it's the same data-admin Find instance either way, just gated on "is IDOL even up".
    case 'find-ui':
      return (document.getElementById('deployTypeBasic')?.checked || false) ||
             (document.getElementById('deployTypeDataAdmin')?.checked || false);
    // NiFi, IDOL UIs, External Tools and the Connector calls are common to every
    // deployment type and are never gated out.
    default:
      return true;
  }
}

function qaEnsureLinkDot(link) {
  let dot = link.nextElementSibling;
  if (!(dot && dot.classList && dot.classList.contains('row-health-dot'))) {
    dot = document.createElement('span');
    dot.className = 'row-health-dot qa-health-unknown';
    link.insertAdjacentElement('afterend', dot);
  }
  return dot;
}

function qaSetLinkStatus(link, status) {
  const dot = qaEnsureLinkDot(link);
  dot.className = `row-health-dot qa-health-${status}`;
  dot.title = { valid: 'Reachable', invalid: 'Unreachable', checking: 'Checking…' }[status] || 'Not checked yet';
}

function qaSetCardStatus(card, status) {
  const dot = card.querySelector('.card-head .qa-health-dot');
  if (!dot) return;
  dot.className = `qa-health-dot qa-health-${status}`;
  dot.title = {
    valid: 'All services in this group are reachable',
    invalid: 'All checked services in this group are unreachable',
    mixed: 'Some services in this group are unreachable',
    checking: 'Validating…',
    inactive: 'Not part of the selected Deployment Type — not validated'
  }[status] || 'Not checked yet';
}

function updateQaSummary({ valid, invalid, pending, checking }) {
  const validEl = document.getElementById('qaValidCount');
  const invalidEl = document.getElementById('qaInvalidCount');
  const pendingPill = document.getElementById('qaPendingPill');
  const pendingEl = document.getElementById('qaPendingCount');
  const lastChecked = document.getElementById('qaLastChecked');
  const btn = document.getElementById('qaRevalidateBtn');

  if (validEl) validEl.textContent = valid;
  if (invalidEl) invalidEl.textContent = invalid;
  if (pendingEl) pendingEl.textContent = pending;
  if (pendingPill) pendingPill.style.display = pending > 0 ? 'inline-flex' : 'none';
  if (btn) {
    btn.disabled = checking;
    btn.style.opacity = checking ? '0.6' : '1';
    btn.style.cursor = checking ? 'not-allowed' : 'pointer';
  }
  if (lastChecked && !checking) {
    lastChecked.textContent = `Last checked ${new Date().toLocaleTimeString()}`;
  }
}

// Checks one URL server-side (avoids CORS / self-signed-cert issues in the browser).
// Returns true/false, or null if a newer validation run has since started (result discarded).
async function qaCheckUrl(url, token) {
  try {
    const response = await fetch('/config-idol/check-nifi', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ url, timeout: QA_VALIDATION_TIMEOUT_S })
    });
    if (token !== qaValidationToken) return null;
    const data = await response.json();
    return !!(data.success && data.reachable);
  } catch {
    return token === qaValidationToken ? false : null;
  }
}

// Validates every real service link in the Quick Access grid, lights up a green/red/orange dot
// on each link, row-unit and card header, and updates the total valid/unreachable counts.
//
// A "unit" is one row's worth of links checked together: a row is "valid" if ANY of its links
// respond, "invalid" only if ALL of them fail. (The "Find" row is why — basic-idol / data-admin
// are alternates, not two independent services, so one reachable link means Find is up.)
// A card is "mixed" (orange) when some of its units are valid and others invalid, "valid" when
// all are, "invalid" when none are, and "inactive" when its Deployment Type isn't selected.
async function validateQuickAccessServices() {
  const grid = document.getElementById('grid');
  if (!grid) return;

  const token = ++qaValidationToken;
  const cards = Array.from(grid.querySelectorAll(':scope > .card'));

  const units = [];
  cards.forEach(card => {
    const cardGroup = card.dataset.qaGroup;
    // A single-group card (everything except the mixed Housekeeping card) that isn't part of
    // the selected Deployment Type is skipped entirely — no network calls, dim "inactive" dot.
    if (cardGroup && cardGroup !== 'housekeeping' && !qaGroupActive(cardGroup)) {
      qaSetCardStatus(card, 'inactive');
      return;
    }
    ...
    rows.forEach(row => {
      if (row.dataset.qaSkip === 'true') return; // e.g. NiFi Registry — intentionally not validated
      const rowGroup = row.dataset.qaGroup; // per-row override, used by the mixed Housekeeping card
      if (rowGroup && !qaGroupActive(rowGroup)) return; // not part of the selected Deployment Type

      const links = Array.from(row.querySelectorAll('a.svc-link'))
        .filter(a => a.getAttribute('href') && a.getAttribute('href') !== '#');
      if (links.length === 0) return;

      checkableCount++;
      links.forEach(link => qaSetLinkStatus(link, 'checking'));
      units.push({ card, links, remaining: links.length, anyValid: false, anyResolved: false });
    });
    qaSetCardStatus(card, checkableCount > 0 ? 'checking' : 'unknown');
  });
  ...
  updateQaSummary({ valid, invalid, pending: totalUnits, checking: true });
  if (totalUnits === 0) { updateQaSummary({ valid: 0, invalid: 0, pending: 0, checking: false }); return; }

  const jobs = [];
  units.forEach(unit => unit.links.forEach(link => jobs.push({ unit, link })));
  let cursor = 0;

  function finalizeUnit(unit) {
    unit.anyValid ? valid++ : invalid++;
    const stats = cardStats.get(unit.card) || { valid: 0, invalid: 0 };
    unit.anyValid ? stats.valid++ : stats.invalid++;
    cardStats.set(unit.card, stats);

    const status = stats.invalid === 0 ? 'valid' : stats.valid === 0 ? 'invalid' : 'mixed';
    qaSetCardStatus(unit.card, status);
    updateQaSummary({ valid, invalid, pending: totalUnits - valid - invalid, checking: true });
  }

  async function worker() {
    while (cursor < jobs.length) {
      const job = jobs[cursor++];
      const ok = await qaCheckUrl(job.link.href, token);
      if (token !== qaValidationToken) return; // superseded by a newer validation run

      if (ok !== null) {
        qaSetLinkStatus(job.link, ok ? 'valid' : 'invalid');
        job.unit.anyResolved = true;
        if (ok) job.unit.anyValid = true;
      }
      job.unit.remaining--;
      if (job.unit.remaining === 0 && job.unit.anyResolved) finalizeUnit(job.unit);
    }
  }

  await Promise.all(Array.from({ length: Math.min(QA_VALIDATION_CONCURRENCY, jobs.length) }, worker));
  if (token !== qaValidationToken) return;

  updateQaSummary({ valid, invalid, pending: 0, checking: false });
  cards.forEach(card => {
    if (cardStats.has(card)) return;
    // A card whose status is still 'checking' here had zero units resolve to a usable
    // true/false (e.g. every request errored ambiguously) — fall back to unknown rather
    // than leaving the pulsing "checking" dot on indefinitely.
    const dot = card.querySelector('.card-head .qa-health-dot');
    if (dot && !dot.classList.contains('qa-health-inactive')) qaSetCardStatus(card, 'unknown');
  });
}
...
function exportAllFieldsToJson() {
  try {
    ...
    const fieldIds = [
      ...
      'communityPort', 'viewServerPort', 'dataadminContentPort',
      'answerServerPort', 'answerBankAgentPort', 'dataadminAgentPort',
  ...
  reader.onload = function(e) {
    try {
      ...
      if (config.deploymentTypes) {
        ...
        // Guard against legacy/corrupt saved configs that have both base
        // deployment types set — Basic IDOL and Data Admin IDOL are mutually
        // exclusive, so Data Admin takes precedence if both were saved true.
        if (basicCheckbox && dataAdminCheckbox && basicCheckbox.checked && dataAdminCheckbox.checked) {
...
// Filters the Quick Access grid by service name or port number as the user types.
// Matching rows stay visible, non-matching rows are hidden, cards with zero visible
// rows are hidden entirely, and cards containing a match are auto-expanded so the
// result isn't hidden behind a collapsed header.
function filterQuickAccess(rawTerm, isClear) {
  const searchInput = document.getElementById('quickAccessSearch');
  const wrapper = searchInput?.closest('.quick-access-search');
  const noResultsBox = document.getElementById('quickAccessNoResults');
  const noResultsTerm = document.getElementById('quickAccessNoResultsTerm');
  const grid = document.getElementById('grid');
  if (!grid) return;

  if (isClear && searchInput) {
    searchInput.value = '';
    searchInput.focus();
  }

  const term = (isClear ? '' : rawTerm || '').trim().toLowerCase();
  ...
  cards.forEach(card => {
    ...
    rows.forEach(row => {
      ...
      row.classList.toggle('qa-hidden', !isMatch && term !== '');
      row.classList.toggle('qa-match', isMatch && term !== '');
      if (isMatch) cardHasMatch = true;
    });

    card.classList.toggle('qa-hidden', term !== '' && !cardHasMatch);
    if (term !== '' && cardHasMatch) anyVisible = true;

    if (term !== '' && cardHasMatch) {
      // Auto-expand so a matched row is actually visible.
      const body = card.querySelector('.card-body');
      const chevron = card.querySelector('.chevron');
      if (body && (body.style.display === 'none' || body.style.display === '')) {
        body.dataset.qaAutoExpanded = 'true';
        body.style.display = 'block';
        chevron?.classList.add('open');
      }
    } else if (term === '') {
      // Restore any card we auto-expanded purely for the search, back to collapsed.
      const body = card.querySelector('.card-body');
      const chevron = card.querySelector('.chevron');
      if (body?.dataset.qaAutoExpanded === 'true') {
        body.style.display = 'none';
        chevron?.classList.remove('open');
        delete body.dataset.qaAutoExpanded;
      }
    }
  });

  if (noResultsBox) {
    noResultsBox.style.display = (term !== '' && !anyVisible) ? 'flex' : 'none';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.querySelectorAll('input[name="deploymentType"]').forEach(el => {
    el.addEventListener('change', () => {
      enforceDeploymentTypeExclusivity(el);
      setupIdolToggleConditionalFields();
      setTimeout(setupIdolReorderNifiPreservePath, 300);
      updateModelVisibility();
      checkBasicNextButton();
      updateLlmWikiVisibility();
      // Which Quick Access cards are relevant changes with the deployment type —
      // re-validate so gated-out groups switch to "inactive" and newly-active ones get checked.
      scheduleQuickAccessValidation();
    });
  });
