...
IDOL_BASE_MOUNT = "/idol-base"
PRE_SETUP_NAME = "pre-setup.sh"
ALLOWED_SAVE_ROOTS = (SCRIPT_PATH, IDOL_BASE_MOUNT)
...
def _resolve_pre_setup_target(requested: str) -> str:
    """
    Prefer /idol-base/pre-setup.sh (directory bind of IDOL_BASE_PATH).
    Fall back to /setup-scripts/pre-setup.sh only if that path is a writable file
    or does not exist yet. Never return a path that is currently a directory.
    """
    idol_target = os.path.join(IDOL_BASE_MOUNT, PRE_SETUP_NAME)
    setup_target = os.path.join(SCRIPT_PATH, PRE_SETUP_NAME)

    requested_abs = os.path.abspath(os.path.normpath(requested or setup_target))
    candidates = []
    if os.path.basename(requested_abs) == PRE_SETUP_NAME or requested_abs.endswith(os.sep + PRE_SETUP_NAME):
        candidates.extend([idol_target, setup_target, requested_abs])
    else:
        candidates.append(requested_abs)
    ...
    for path in candidates:
        ...
        if os.path.isdir(parent) and os.access(parent, os.W_OK):
            return path
        if path == idol_target and os.path.isdir(IDOL_BASE_MOUNT):
            return path
    return idol_target if os.path.isdir(IDOL_BASE_MOUNT) else setup_target
...
def save_script():
    try:
        ...
        file_path = data.get('file_path', f'{IDOL_BASE_MOUNT}/{PRE_SETUP_NAME}')
        script_content = data.get('script_content', '')
        if not file_path or script_content is None or script_content == '':
        ...
        requested_abs = os.path.abspath(os.path.normpath(file_path))
        target_abs = _resolve_pre_setup_target(requested_abs) if os.path.basename(requested_abs) == PRE_SETUP_NAME else requested_abs

        if not any(_is_under_root(target_abs, root) for root in ALLOWED_SAVE_ROOTS):
            log_with_context(logging.ERROR, f"Refusing to save script outside {ALLOWED_SAVE_ROOTS}: {target_abs}")
            return jsonify({'success': False, 'error': f'Script path must be under {ALLOWED_SAVE_ROOTS}'}), 400

        if os.path.isdir(target_abs):
            msg = (
                f"Cannot save script: '{target_abs}' is a directory. "
                "Docker created that directory because a file bind source was missing. "
                "On the host: docker compose down, then "
                "rm -rf \"$IDOL_BASE_PATH/pre-setup.sh\" && "
                "./deploy-setup-manager-ui.sh --build"
            )
            log_with_context(logging.ERROR, msg)
            return jsonify({'success': False, 'error': msg}), 500

        parent_dir = os.path.dirname(target_abs) or SCRIPT_PATH
        try:
            os.makedirs(parent_dir, exist_ok=True)
        except OSError as e:
            log_with_context(logging.ERROR, f"Cannot create script directory {parent_dir}: {e}")
            return jsonify({'success': False, 'error': f'Cannot create script directory: {e}'}), 500

        if not os.access(parent_dir, os.W_OK):
            log_with_context(logging.ERROR, f"No write permission for directory: {parent_dir}")
            return jsonify({'success': False, 'error': f'No write permission for directory: {parent_dir}'}), 403
        try:
            ...
            try:
                ...
                pass
            log_with_context(logging.INFO, f"Script saved successfully to {target_abs}")
            return jsonify({
                'success': True,
            ...
            return jsonify({
                'success': False,
                'error': (
                    f"Error saving script: {str(e)}. "
                    "Stop the UI container, delete the host directory named pre-setup.sh, and redeploy."
                )
            }), 500
        except Exception as e:
            log_with_context(logging.ERROR, f"Error saving script: {str(e)}")
...
def serve_llm_models():
    ...
    if os.path.isdir(file_path):
        # Same class of bug as pre-setup.sh: Docker bind-mounted a missing
        # host FILE as a directory because the host source path didn't
        # exist (or was wrong) at container start.
        msg = (
            f"'{file_path}' is a directory, not the models file. Docker creates a "
            "directory when the host source file for a bind mount doesn't exist. "
            "Check that $IDOL_BASE_PATH/idol-containers-toolkit/data-admin/llm-sandbox/"
            "default-models.json exists on the host, then: docker compose down, "
            "rm -rf that path if it's a directory, put the real file back, and redeploy."
        )
        log_with_context(logging.ERROR, msg)
        return jsonify({"error": "default-models.json mount is a directory", "detail": msg}), 500
    ...
    if not os.path.exists(file_path):
        return jsonify({
            ...
            "detail": (
                ...
                "→ /setup-scripts/default-models.json"
            )
        }), 404
    ...
    try:
        ...
        msg = f"'{file_path}' exists but is not valid JSON: {e}"
        log_with_context(logging.ERROR, msg)
        return jsonify({"error": "default-models.json is not valid JSON", "detail": msg}), 500
