...
PORT="${SETUP_UI_PORT:-}"
CMD=""
# Guards against looping: automatic mount repair is attempted at most once
# per script invocation. If it doesn't fix things, we stop and tell the
# user to investigate instead of rebuilding forever.
...
# Runs verify_idol_base_mount(); if it finds a broken /idol-base mount or a
# stray /setup-scripts/pre-setup.sh directory, automatically runs the same
# repair that --fix-mounts does, then re-verifies. Only ever attempts this
# once per invocation (AUTO_FIX_ATTEMPTED guard) so a persistent problem
# fails loudly instead of rebuilding in a loop.
verify_and_autofix() {
    if verify_idol_base_mount; then
        return 0
    fi

    if [ "$AUTO_FIX_ATTEMPTED" = "1" ]; then
        error "Mount problem persists after automatic repair."
        error "Run '$(basename "$0") --fix-mounts --port $PORT' manually, then check:"
        error "  docker inspect $SERVICE_NAME --format '{{range .Mounts}}{{.Source}} -> {{.Destination}}{{println}}{{end}}'"
        return 1
    fi

    AUTO_FIX_ATTEMPTED=1
    warn "Mount problem detected after deploy — attempting automatic repair."
    warn "This will stop the container, remove any stray pre-setup.sh directory, rebuild, and redeploy."
    fix_pre_setup_mounts

    if verify_idol_base_mount; then
        success "Automatic repair fixed the mount issue."
        return 0
    fi

    error "Automatic repair did not resolve the mount issue."
    error "Run '$(basename "$0") --fix-mounts --port $PORT' manually and inspect the container mounts."
    return 1
}
...
deploy_only() {
    ...
    if docker image inspect idol-config-nifi-manager >/dev/null 2>&1; then
        if [ ! -f "$DOCKERFILE" ]; then
            error "Image 'idol-config-nifi-manager' exists but Dockerfile not found at: $DOCKERFILE"
            exit 1
        fi
        warn "Image 'idol-config-nifi-manager' already exists locally."
        warn "Removing it and rebuilding from $DOCKERFILE (context $PROJECT_ROOT)..."
        docker rmi -f idol-config-nifi-manager >/dev/null 2>&1 || true
        ...
        info "Image 'idol-config-nifi-manager' not found locally."
        info "Building it now from $DOCKERFILE (context $PROJECT_ROOT)..."
        docker build -f "$DOCKERFILE" -t idol-config-nifi-manager "$PROJECT_ROOT"
    fi
