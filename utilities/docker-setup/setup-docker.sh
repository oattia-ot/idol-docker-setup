...
# ======================================================
# Docker & Docker Compose Setup Script for Ubuntu / macOS
# Host OS is auto-detected via module/os-compat.sh
# ======================================================
...
_SETUP_DOCKER_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
_IDOL_ROOT="$(cd "$_SETUP_DOCKER_DIR/../.." && pwd)"
...
install_docker() {
    ...
    if [ "$(uname -s)" = "Darwin" ]; then
        if command -v brew >/dev/null 2>&1; then
            ...
            open -a Docker || true
            log "${CALLING_SCRIPT} ✅${GREEN}Docker Desktop installed. Wait for the engine, then re-run.${NC}"
            return 0
        fi
        log "${CALLING_SCRIPT} ${RED}Install Docker Desktop from https://docs.docker.com/desktop/setup/install/mac-install/${NC}"
        return 1
    fi
...
install_docker_compose() {
    ...
    # Create symbolic link for command completion (Linux only; /usr/bin is SIP-protected on macOS)
    if [ "$(uname -s)" != "Darwin" ]; then
