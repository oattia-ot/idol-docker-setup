...
if [ "${IDOL_OS_FAMILY:-}" = "macos" ] || [ "$(uname -s)" = "Darwin" ]; then
    echo "[OS] macOS detected. systemd containerd/docker unit repair is Linux-only."
    echo "     Restart Docker Desktop instead:  osascript -e 'quit app \"Docker\"' && open -a Docker"
    exit 0
fi
