#!/usr/bin/env bash
# Functions to verify/install Docker, kubectl, helm, minikube, jq
# Host OS is auto-detected (ubuntu / macos).

_PRE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
_IDOL_ROOT="$(cd "$_PRE_DIR/../../.." && pwd)"
if [ -f "$_IDOL_ROOT/module/os-compat.sh" ]; then
    # shellcheck source=/dev/null
    . "$_IDOL_ROOT/module/os-compat.sh"
fi

check_prerequisites() {
    log "INFO" "Checking prerequisites..."

    # Docker
    if ! check_command docker; then
        log "WARN" "Docker not found. Installing..."
        install_docker
    else
        log "INFO" "Docker is already installed."
    fi

    # kubectl
    if ! check_command kubectl; then
        log "WARN" "kubectl not found. Installing..."
        install_kubectl
    fi

    # helm
    if ! check_command helm; then
        log "WARN" "helm not found. Installing..."
        install_helm
    fi

    # minikube
    if ! check_command minikube; then
        log "WARN" "minikube not found. Installing..."
        install_minikube
    fi

    # jq (for JSON parsing)
    if ! check_command jq; then
        log "WARN" "jq not found. Installing..."
        if command -v idol_pkg_install >/dev/null 2>&1; then
            idol_pkg_install jq
        elif [ "$(uname -s)" = "Darwin" ]; then
            brew install jq
        else
            sudo apt-get update && sudo apt-get install -y jq
        fi
    fi

    log "INFO" "All prerequisites satisfied."
}

install_docker() {
    if command -v idol_install_docker >/dev/null 2>&1; then
        idol_install_docker
        return $?
    fi
    if [ "$(uname -s)" = "Darwin" ]; then
        brew install --cask docker
        open -a Docker || true
        return 0
    fi
    # Official Docker install script (Linux)
    curl -fsSL https://get.docker.com -o get-docker.sh
    sudo sh get-docker.sh
    rm get-docker.sh
    sudo usermod -aG docker "$USER"
    log "WARN" "You may need to log out and back in for docker group changes to take effect."
    sudo systemctl enable docker && sudo systemctl start docker
}

install_kubectl() {
    if command -v idol_install_kubectl >/dev/null 2>&1; then
        idol_install_kubectl
        return $?
    fi
    if [ "$(uname -s)" = "Darwin" ]; then
        brew install kubectl
        return $?
    fi
    local version arch="amd64"
    version=$(curl -L -s https://dl.k8s.io/release/stable.txt)
    case "$(uname -m)" in aarch64|arm64) arch="arm64" ;; esac
    curl -LO "https://dl.k8s.io/release/$version/bin/linux/${arch}/kubectl"
    chmod +x kubectl
    sudo install -o "$USER" -g "$USER" -m 0755 kubectl /usr/local/bin/kubectl
    rm kubectl
}

install_helm() {
    if [ "$(uname -s)" = "Darwin" ] && command -v brew >/dev/null 2>&1; then
        brew install helm
        return $?
    fi
    curl -fsSL -o get_helm.sh https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3
    chmod 700 get_helm.sh
    ./get_helm.sh
    rm get_helm.sh
}

install_minikube() {
    if command -v idol_install_minikube >/dev/null 2>&1; then
        idol_install_minikube
        return $?
    fi
    if [ "$(uname -s)" = "Darwin" ]; then
        brew install minikube
        return $?
    fi
    local arch="amd64"
    case "$(uname -m)" in aarch64|arm64) arch="arm64" ;; esac
    curl -LO "https://storage.googleapis.com/minikube/releases/latest/minikube-linux-${arch}"
    sudo install minikube-linux-${arch} /usr/local/bin/minikube
    rm -f minikube-linux-${arch}
}
