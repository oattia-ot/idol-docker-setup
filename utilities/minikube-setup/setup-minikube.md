...
## Overview
`deploy-minikube.sh` is a comprehensive deployment script for managing Minikube clusters on **Ubuntu 24.04** and **macOS**. It auto-detects the host OS (`module/os-compat.sh`), installs prerequisites (Docker, kubectl, helm, jq) with `apt` or Homebrew, and handles cluster startup, add‑on configuration, and registry secrets.

## Prerequisites
- Ubuntu 24.04 (or similar Debian‑based system) **or macOS** (Intel or Apple silicon)
- `sudo` privileges on Ubuntu; Homebrew + Docker Desktop on macOS
- Internet connection to download packages and container images
- The script will install missing dependencies automatically:
  - Ubuntu: Docker Engine + systemd, `apt-get install jq`, Linux kubectl/minikube binaries
  - macOS: Docker Desktop (`brew install --cask docker`), `brew install kubectl helm minikube jq`
- Preferred one-shot host install (includes optional helm/kubectl/minikube):
