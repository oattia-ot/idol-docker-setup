...
cleanup_on_error() {
    ...
    if docker compose -f "$IDOL_LICENSE_SERVER_PATH/docker-compose.yml" ps -q licenseserver 2>/dev/null | grep -q .; then
        log_info "Stopping license server container..."
        docker compose -f "$IDOL_LICENSE_SERVER_PATH/docker-compose.yml" down || true
    fi
}
...
load_environment() {
    ...
    if [ -z "${IDOL_LICENSE_SERVER_PATH:-}" ]; then
        ...
        log_error "  export IDOL_LICENSE_SERVER_PATH=/path/to/licenseserver-setup"
        log_error "  OR"
        log_error "  source env/licenseserver-path.env"
        exit 1
    fi
...
validate_prerequisites() {
    ...
    if [ ! -f "$IDOL_LICENSE_SERVER_PATH/docker-compose.yml" ]; then
        log_error "docker-compose.yml not found at: $IDOL_LICENSE_SERVER_PATH/docker-compose.yml"
        exit 1
    fi
...
start_license_server() {
    ...
    if docker compose -f "$IDOL_LICENSE_SERVER_PATH/docker-compose.yml" up -d --build licenseserver; then
...
wait_for_service() {
    ...
    docker compose -f "$IDOL_LICENSE_SERVER_PATH/docker-compose.yml" logs licenseserver --tail=20 || true
...
show_status() {
    ...
    docker compose -f "$IDOL_LICENSE_SERVER_PATH/docker-compose.yml" ps
